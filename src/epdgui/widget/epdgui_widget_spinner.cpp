...
void EPDGUI_Widget_Spinner::Render(JsonVariant data)
{
    ...
    RenderLeftButton(this->_leftButton->CanvasNormal(), false, lowerIcon);
    RenderLeftButton(this->_leftButton->CanvasPressed(), true, lowerIcon);

    RenderRightButton(this->_rightButton->CanvasNormal(), false, lowerIcon);
    RenderRightButton(this->_rightButton->CanvasPressed(), true, lowerIcon);
}
...
void EPDGUI_Widget_Spinner::RenderLeftButton(M5EPD_Canvas *canvas, bool pressed, String icon)
{
    ...
    canvas->drawJpgFile(SD, icon.c_str(), canvas->width() / 2 - 40, canvas->height() / 2, 60, 60);
